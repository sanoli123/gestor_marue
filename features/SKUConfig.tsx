...
import { SKUSegmentOption } from '../types.ts';
...
// Fallback para gerar IDs caso crypto.randomUUID não exista no browser
function genId() {
...
const SegmentManager: React.FC<SegmentManagerProps> = ({ title, segmentKey }) => {
  ...
  // ✅ defensivo: se vier null/objeto, garantimos array
  const segmentData: SKUSegmentOption[] =
    skuConfig && Array.isArray((skuConfig as any)[segmentKey])
      ? ((skuConfig as any)[segmentKey] as SKUSegmentOption[])
      : [];
  ...
  const handleSave = async () => {
    ...
    const code = (currentItem?.code ?? '').toString().trim().toUpperCase();

    if (!skuConfig) {
      alert('Configuração de SKU ainda não carregou.');
      return;
    }
    if (!name || !code) {
    ...
    // normaliza item
    const normalized: SKUSegmentOption = {
      id: currentItem?.id || genId(),
      name,
      code,
    };

    // atualiza lista (edit ou add)
    const updatedSegment = currentItem?.id
      ? segmentData.map((item) => (item.id === currentItem.id ? normalized : item))
      : [...segmentData, normalized];

    try {
      await updateSkuConfig({ ...skuConfig, [segmentKey]: updatedSegment } as any);
      closeModal();
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (
      window.confirm(
        'Tem certeza que deseja excluir este item? Isso pode afetar a geração de futuros SKUs.'
      )
    ) {
  ...
  return (
    <div className="bg-lino p-6 rounded-xl shadow-lg">
        ...
        <table className="w-full text-sm text-left text-oliva">
          ...
          <tbody>
            {segmentData.map((item) => {
              // ✅ key estável e garantida
              const key = item.id || `${item.code}-${item.name}`;
              return (
                <tr key={key} className="bg-crema border-b border-lino hover:bg-lino">
            ...
            {segmentData.length === 0 && (
              <tr>
                <td className="px-6 py-6 text-sm text-oliva" colSpan={3}>
                  Nenhum item cadastrado ainda. Clique em <b>Adicionar</b> para incluir o primeiro.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={`${currentItem?.id ? 'Editar' : 'Adicionar'} Item`}
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            onChange={(e) =>
              setCurrentItem((prev) => ({ ...prev, code: e.target.value.toUpperCase() }))
            }
            maxLength={4}
          />
...
const SKUConfigPage: React.FC = () => {
  return (
    <div className="space-y-8">
      ...
      <p className="text-oliva mt-1">
        Gerencie os códigos usados para gerar os SKUs dos seus produtos automaticamente.
      </p>
